...
class PolicyActorNetwork:
    def __init__(self, state_size, action_size, name='policy_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.W1 = tf.get_variable("W1_" + game_name, [self.state_size, policy_num_n], initializer=kernel_initializer)
            self.b1 = tf.get_variable("b1_" + game_name, [policy_num_n], initializer=tf.zeros_initializer())
            self.W2 = tf.get_variable("W2_" + game_name, [policy_num_n, policy_num_n], initializer=kernel_initializer)
            self.b2 = tf.get_variable("b2_" + game_name, [policy_num_n], initializer=tf.zeros_initializer())
            self.W3 = tf.get_variable("W3_" + game_name, [policy_num_n, self.action_size], initializer=kernel_initializer)
            self.b3 = tf.get_variable("b3_"+game_name, [self.action_size], initializer=tf.zeros_initializer())
...
class ValueCriticNetwork:
    def __init__(self, state_size, action_size, learning_rate, name='value_network'):
        ...
        with tf.variable_scope(name):
            ...
            self.W1 = tf.get_variable("W1_"+game_name, [self.state_size, value_num_n],initializer=kernel_initializer)
            self.b1 = tf.get_variable("b1_"+game_name, [value_num_n], initializer=tf.zeros_initializer())
            self.W2 = tf.get_variable("W2_"+game_name, [value_num_n, value_num_n], initializer=kernel_initializer)
            self.b2 = tf.get_variable("b2_"+game_name, [value_num_n], initializer=tf.zeros_initializer())
            self.W3 = tf.get_variable("W3_"+game_name, [value_num_n, 1], initializer=kernel_initializer)
            self.b3 = tf.get_variable("b3_"+game_name, [1], initializer=tf.zeros_initializer())
...
with tf.Session() as sess:
    ...
    for episode in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            action = np.random.choice([0] * int((action_size/2)) + [1] * int((action_size/2)), p=actions_distribution)
            ...
            action_one_hot = [1 - action] * int((action_size/2)) + [action] * int((action_size/2))
